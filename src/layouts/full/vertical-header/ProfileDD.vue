        ...
        <template v-slot:activator="{ props }">
            <div class=" text-left px-0 cursor-pointer" variant="text" v-bind="props">
                <div class="d-flex align-center">
                    <v-avatar size="50">
                        <img src="@/assets/images/profile/user6.jpg" width="50" alt="{{ authStore.user?.fullName }}" />
                    </v-avatar>
                    <div class="ml-md-4 d-md-block d-none">
                        <h6 class="text-h6 d-flex align-center text-black font-weight-semibold">{{ authStore.user?.fullName }}</h6>
                        <span class="text-subtitle-2 font-weight-medium text-grey100">{{ authStore.user?.rol }}</span>
                    </div>
                </div>
            </div>
        </template>
        <v-sheet rounded="lg" width="385" elevation="10" class="mt-5">
            <div class="px-8 pt-6">
                <div class="d-flex align-center justify-space-between">
                    <h6 class="text-h5 font-weight-semibold">Perfil de usuario</h6>
                ...
                <div class="d-flex align-center mt-5 pb-6">
                    ...
                    <div class="ml-5">
                        <h6 class="text-h5 mb-n1">{{ authStore.user?.username || 'Usuario' }}</h6>
                        ...
                        <div class="d-flex align-center mt-1">
                            ...
                            <span class="text-subtitle-1 text-grey100 font-weight-medium ml-2">{{ authStore.user?.email || 'correo@desconocido.com' }}</span>
                        </div>
                    </div>
                </div>
                <v-divider></v-divider>
